...
//   dirname: "",
//   filename: "",
//   sourcename: "agent-group-outputs.js"
// });
...
const getAgentGroupOutputs = async ({ date_unit, tenant_key, site_cd, agent_group, start_date, end_date, start_time, end_time }) => {
	...
	SELECT 
            ROW_NUMBER() OVER(ORDER BY ISNULL(ISNULL(T1.DT_KEY, T2.DT_KEY), T3.DT_KEY), ISNULL(ISNULL(T1.GROUP_NAME, T2.GROUP_NAME), T3.GROUP_NAME)) AS NUM,
			...
			, ISNULL(ISNULL(T1.GROUP_NAME, T2.GROUP_NAME), T3.GROUP_NAME) AS GROUP_NAME
			...
			-- , IB_OFFERED, IB_FAILED, IB_ENGAGE, IB_ENGAGE_TIME
			-- , IB_TRANSFER_INIT_AGENT, IB_XFER_RECEIVED_ACCEPTED 

			-- , OB_OFFERED, OB_FAILED, OB_ENGAGE, OB_ENGAGE_TIME
			-- , OB_TRANSFER_INIT_AGENT, OB_XFER_RECEIVED_ACCEPTED

			-- , IN_OFFERED, IN_FAILED, IN_ENGAGE, IN_ENGAGE_TIME
			-- , IN_TRANSFER_INIT_AGENT, IN_XFER_RECEIVED_ACCEPTED
          ...
          -- ROW_NUMBER() OVER(ORDER BY A.GROUP_NAME, A.DT_KEY, A.M_TYPE, A.I_TYPE) AS NUM
	      -- , A.DT_KEY , A.GROUP_NAME , A.M_TYPE , A.I_TYPE , A.OFFERED , A.FAILED , A.ENGAGE , A.ENGAGE_TIME , A.TRANSFER_INIT_AGENT , A.XFER_RECEIVED_ACCEPTED
	  ...
	  FROM (SELECT (SELECT CONVERT(${date_type}, DT.CAL_DATE, 20)
					...
					, (SELECT GROUP_NAME
					FROM GROUP_ G
					WHERE G.GROUP_KEY = B.GROUP_KEY) AS GROUP_NAME
           ...
           FROM ${main_view_name} A INNER JOIN
                   (SELECT DISTINCT A.RESOURCE_KEY, B.GROUP_KEY
           ...
           GROUP BY B.GROUP_KEY, A.DATE_TIME_KEY, A.MEDIA_TYPE_KEY, A.INTERACTION_TYPE_KEY
           ) T1
           ...
           (SELECT (SELECT CONVERT(${date_type}, DT.CAL_DATE, 20)
            ...
            ON T1.DT_KEY = T2.DT_KEY AND T1.GROUP_NAME = T2.GROUP_NAME
           ...
           (SELECT (SELECT CONVERT(${date_type}, DT.CAL_DATE, 20)
            ...
            ON (T1.DT_KEY = T3.DT_KEY AND T1.GROUP_NAME = T3.GROUP_NAME) 
                OR (T2.DT_KEY = T3.DT_KEY AND T2.GROUP_NAME = T3.GROUP_NAME)
    ) X
    WHERE 1=1
    ${time_range_query}
    ;`
    ...
    // const query = await generateAgentGroupOutputsQuery({ date_unit, tenant_key, site_cd, agent_group, start_date, end_date })
    // const parameters = [];
  
    const rows = await sendPreparedStatementToInfomart(query, parameters);

    console.log("[agent-group-outputs] query: " + query + "\n    parameters: " + JSON.stringify(parameters));
    console.log("[agent-group-outputs] query result: " + JSON.stringify(rows));
    return rows;
        // .then(result => {
