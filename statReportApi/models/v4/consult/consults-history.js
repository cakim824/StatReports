...
const getConsultsHistory = async ({ date_unit, site_cd, start_date, end_date, start_time, end_time, agent_group, agent_id, empno_list, media_type, interaction_type, main_category, medium_category, sub_category }) => {
    ...
    SELECT 
		...
		(SELECT COM_ENM FROM tb_scode WHERE COM_SCD = A.CHANNEL) AS CHANNEL, 
		(SELECT COM_ENM FROM tb_scode WHERE COM_SCD = A.IO_GUBUN) AS INTERACTION,	
		A.CD01, A.CD02, A.CD03,
        COUNT(*) AS COUNT,
    ...
    FROM (
	    ...
	    SELECT
			...
			CUSTDATA_SEQ, CALL_NUMBER, CHANNEL, IO_GUBUN, CD01, CD02, CD03, CRT_DATE, CRT_EMP_NO, CALL_ID, CALL_UUID
	    FROM tb_consult
