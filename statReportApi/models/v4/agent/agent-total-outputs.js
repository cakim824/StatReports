...
const { filterDateArguments } = require('../../../utils/common');
...
const getAgentOutputs = async ({ date_unit, tenant_key, site_cd, start_date, end_date, start_time, end_time, inbound_type_keys, outbound_type_keys }) => {

    const main_view_name = getViewName(date_unit);
    const date_type = getDateType(date_unit);

    var start_timestamp = getTimestamp(filterDateArguments(start_date)); 
    var end_timestamp = getTimestamp(filterDateArguments(end_date)) + 85500;
    ...
    SELECT  ROW_NUMBER() OVER(ORDER BY X.DT_KEY) AS NUM
            ...
            , SUM(X.IB_OFFERED) AS IB_OFFERED_CNT, SUM(X.IB_ENGAGE) AS IB_ENGAGE_CNT
            , ISNULL(ROUND(SUM(X.IB_ENGAGE)/SUM(X.IB_OFFERED)*100, 2), 0) AS IB_RESPONSE_RATE
            , SUM(X.OB_OFFERED) AS OB_OFFERED_CNT, SUM(X.OB_ENGAGE) AS OB_ENGAGE_CNT
